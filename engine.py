...
class DDP_Engine():
    ...
    def get_tqdm_postfix(self):
        ...
        info['Tumor'] = f"{self.tracer.mode['train'].counter['dice_tumor'].out()*100:.2f}%"
    ...
    def train(self):
        ...
        for batch_id, batch in enumerate(self.datasets.train_loader,1):
            ...
            for i in range(self.args.mini_batch):
                ...
                data = self.all_gather_data( self.model.train(image, label, batch_id % self.args.acm_grad==0 or batch_id==len(self.datasets.train_loader)) )
    ...
    def visualize_and_save(self, epoch):
        ...
        if self.local_rank == 0:
            ...
            if (self.start_epoch+epoch+1) % self.args.save_freq == 0:
                self.model.save()
                self.tracer.save()
